...
"MSWin32-x64-multi-thread-10.0.19041.329" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.19041.329) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAELOOP WSAECONNRESET WSAESOCKTNOSUPPORT WSAECONNABORTED WSAEISCONN
	WSAENOPROTOOPT WSAECANCELLED WSAEALREADY WSAENETUNREACH WSAEREMOTE
	WSAEPROTONOSUPPORT WSAEMFILE WSAENOTSOCK WSAEUSERS WSAEWOULDBLOCK
	WSAEINVAL WSAEPROTOTYPE WSAEHOSTUNREACH WSAENETRESET WSAEADDRINUSE
	WSAESHUTDOWN WSAEINTR WSAENOTCONN WSAESTALE WSAETIMEDOUT WSAENETDOWN
	WSAEINVALIDPROCTABLE WSAEACCES WSAEAFNOSUPPORT WSAENOBUFS
	WSAEINPROGRESS WSAEDESTADDRREQ WSAECONNREFUSED WSAENAMETOOLONG
	WSAEADDRNOTAVAIL WSAEMSGSIZE WSAEPFNOSUPPORT WSAEINVALIDPROVIDER
	WSAEBADF WSAEOPNOTSUPP WSAEDISCON WSAEFAULT WSAEPROVIDERFAILEDINIT
	WSAEPROCLIM WSAEHOSTDOWN WSAENOMORE WSAENOTEMPTY WSAEDQUOT WSAEREFUSED
	WSAETOOMANYREFS
    )],
);
